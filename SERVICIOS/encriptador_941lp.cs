...
namespace SERVICIOS
{
    public class encriptador_941lp
    {
        ...
        {
            using (SHA256 sha256_941lp = SHA256.Create())
            {
                byte[] stream_941lp = sha256_941lp.ComputeHash(Encoding.UTF8.GetBytes(str_941lp));
                StringBuilder sb_941lp = new StringBuilder();
                foreach (byte b_941lp in stream_941lp)
                    sb_941lp.AppendFormat("{0:x2}", b_941lp);
                return sb_941lp.ToString();
            }
        }
        #endregion
